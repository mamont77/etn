...
class typografica
{
  ...
  {
    $this->wacko = &$wacko;
    $this->Indent1 = "<img src=".$wacko->images_dir.$this->Indent1;
    $this->Indent2 = "<img src=".$wacko->images_dir.$this->Indent2;
  }
  ...
  function replaceSpecials( $data )
  {
    ...
    {
      $data = preg_replace ("/(\S+)-(\S+)/", "<nobr>\\1&ndash;\\2</nobr>", $data);
    }
    
    // Имена и фамилии
    $data = preg_replace ("/([А-ЯA-Z])([\. ]{1})[ ]{0,1}([А-ЯA-Z])([\. ]{1})[ ]{0,1}([А-ЯA-Z][А-Яа-яA-Za-z]*)/", '\\1.&nbsp;\\3.&nbsp;\\5', $data);

    // Пушкин А. С.
    $data = preg_replace ("/([А-ЯA-Z][А-Яа-яA-Za-z]*) ([А-ЯA-Z])[\. ]{1}[ ]{0,1}([А-ЯA-Z])\.([,\ )]{1})/", '\\1&nbsp;\\2.&nbsp;\\3.\\4', $data);
    ...
    if ($this->settings["emdash"])
    {
      ...
      $data = preg_replace( "/([ ]+|&nbsp;)(\-\-|\-)\s+/i", "&nbsp;&mdash; ", $data );
    }
